...
It targets JDK 27+, uses named Java modules, and has no third-party runtime
dependencies.
...
JDK 27 or newer is required. The latest line is the tip of Oracle's
...
Release `1.16.1` is the current line:

```kotlin
dependencies {
    implementation("no.beint.skaldpdf:skald-layout:1.16.1")
    implementation("no.beint.skaldpdf:skald-fonts:1.16.1")         // direct SkaldSans API access
    implementation("no.beint.skaldpdf:skald-barcode:1.16.1")        // optional symbols
    implementation("no.beint.skaldpdf:skald-invoice-no:1.16.1")    // optional Norwegian invoice
    implementation("no.beint.skaldpdf:skald-label-sticker:1.16.1") // optional clothing stickers
    implementation("no.beint.skaldpdf:skald-sign:1.16.1")          // optional integrity seals
    implementation("no.beint.skaldpdf:skald-image:1.16.1")         // optional JPEG/PNG/GIF/BMP/HEIC/JXL ingest
    implementation("no.beint.skaldpdf:skald-optimize:1.16.1")      // optional received-PDF recompress
    implementation("no.beint.skaldpdf:skald-optimize-jpegli:1.16.1") // optional bundled JPEGli encoder
}
```
...
JPEGs, quality 90 for lossless rasters, a 20-megapixel decode ceiling, a 16 KiB
lossless-raster work floor, and 2% / 4 KiB savings gates. It also compresses
raw and ASCII-armoured streams, selectively improves weak Flate streams, and
shares byte-identical simple image XObjects and embedded font program streams
without changing decoded bytes.
...
policy byte-for-byte idempotent. See the
[optimizer safety model](docs/pdf-optimization-safety.md) and
[private corpus workflow](docs/private-pdf-corpus.md).
...
[PDF optimization safety](docs/pdf-optimization-safety.md),
