...
class ApplicationUseCaseRenderingTest {
    ...
    private static final DeviceRgb PALE_GREEN = new DeviceRgb(238, 245, 241);
    ...
    private byte[] invoiceOrderOffer() {
        return create(PageSize.A4, 40, "Invoice 2026-1001", (pdf, document) -> {
            ...
            document.add(new Paragraph("Northstar Ledger AS\nKarl Johans gate 1\n0154 Oslo").setFontSize(10));
            ...
            document.add(new Paragraph("Due 26.08.2026 · Account 1503.45.67890").bold().setMarginTop(14));
        });
    }
    ...
    private byte[] invoiceReminder() {
        return create(PageSize.A4, 40, "Payment reminder", (pdf, document) -> {
            ...
            document.add(new Paragraph("Amount due NOK 10 117.47").bold().setFontSize(14).setMarginTop(16));
        });
    }
    ...
    private byte[] agreements() {
        return create(PageSize.A4, 44, "SERVICE AGREEMENT", (pdf, document) -> {
            ...
            document.add(new Paragraph("Between Northstar Advisory AS and Nordlys Handel AS")
                .setTextAlignment(TextAlignment.CENTER));
            for (int section = 1; section <= 14; section++) {
                var block = new Div().setKeepTogether(true);
                block.add(new Paragraph(section + ". Section " + section).bold().setFontSize(12).setKeepWithNext(true));
            ...
            document.add(new Paragraph("Signatures").bold().setFontSize(14).setMarginTop(18));
            var signatures = new Table(UnitValue.createPercentArray(new float[] { 1, 1 })).useAllAvailableWidth();
            row(signatures, "For Northstar Advisory AS\n\n____________________",
                "For Nordlys Handel AS\n\n____________________");
    ...
    private byte[] annualAccountNotes() {
        return create(PageSize.A4, 48, "Notes to the annual accounts", (pdf, document) -> {
            ...
            for (var note : List.of(
                ...
                document.add(new Paragraph(parts[0]).bold().setFontSize(13).setMarginTop(10).setKeepWithNext(true));
    ...
    private byte[] posReceipt() {
        return create(new PageSize(226.77f, 841.89f), 13, "SALES RECEIPT", (pdf, document) -> {
            document.add(new Paragraph("SALES RECEIPT").bold().setFontSize(15).setTextAlignment(TextAlignment.CENTER));
            document.add(new Paragraph("Northstar Coffee · Terminal 2\nReceipt 0001042 · 12.08.2026 07:43")
                .setFontSize(8));
            ...
            document.add(new Paragraph("TOTAL NOK 252.00").bold().setFontSize(12).setTextAlignment(TextAlignment.RIGHT));
    ...
    private byte[] zReport() {
        return create(PageSize.A4, 30, "Z REPORT", (pdf, document) -> {
            ...
            document.add(totals);
            document.add(new Paragraph("Payment methods").bold().setFontSize(13).setMarginTop(16));
    ...
    private byte[] payslips() {
        return create(PageSize.A4, 32, "PAYSLIP", (pdf, document) -> {
            for (int employee = 1; employee <= 2; employee++) {
                ...
                document.add(new Paragraph("Employee " + employee + " · Northstar Ledger AS"));
                ...
                document.add(lines.setMarginTop(18));
                document.add(new Paragraph("Net pay NOK 37 400").bold().setFontSize(14).setTextAlignment(TextAlignment.RIGHT));
                document.add(new Paragraph("Year to date").bold().setFontSize(12).setMarginTop(18));
            }
        });
    }
    ...
    private byte[] taxReturnReceipt() {
        return create(PageSize.A4, 28, "Tax return receipt", (pdf, document) -> {
            ...
            document.add(new Paragraph("Validation result").bold().setFontSize(12).setMarginTop(14));
    ...
    private byte[] stockList() {
        return create(PageSize.A4, 36, "Stock list", (pdf, document) -> {
            ...
            document.add(new Paragraph("Total stock value: NOK 184 420.50").bold().setFontSize(12));
    ...
    private byte[] genericReport() {
        return create(PageSize.A3.rotate(), 32, "General ledger", (pdf, document) -> {
            ...
            var report = table(new float[] { 9, 10, 10, 34, 12, 12, 13 },
                "Date", "Voucher", "Account", "Description", "Debit", "Credit", "Balance");
    ...
    private byte[] create(PageSize pageSize, float margin, String title,
        ...
        var output = new ByteArrayOutputStream();
        var pdf = new PdfDocument(new PdfWriter(output));
        pdf.getDocumentInfo().setTitle(title).setAuthor("Skald PDF");
    ...
    private static Paragraph title(String text) {
        return new Paragraph(text).bold().setFontSize(19).setFontColor(GREEN).setMarginBottom(12);
    }
    ...
    private static Table table(float[] widths, String... headers) {
        ...
        for (var header : headers) {
            table.addHeaderCell(new Cell().add(new Paragraph(header).bold().setFontSize(8))
                ...
                .setBorder(new SolidBorder(GREEN, 0.5f)).setPadding(5));
        }
        return table;
    }
