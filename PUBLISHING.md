...
Skald publishes `skald-core`, `skald-layout`, `skald-barcode`, `skald-labels`,
`skald-sign`, `skald-image`, and `skald-optimize` to Maven Central under
`no.beint.skaldpdf`. Java packages remain `org.skaldpdf`.
