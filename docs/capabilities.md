...
| Text | Unicode, embedded subset TrueType, PostScript font names, regular/bold, color, left/center/right/justify, rotation, underline, strikethrough, `.notdef` for missing glyphs |
| Layout | Paragraphs, lists, runs, margins, padding, backgrounds, gradients, rounded borders, dashed rules, divs, nested cell content, fixed position, `keepTogether`, fitted-image measure, `AreaBreak` page size |
| Tables | weighted columns, spans, repeating headers, page fragmentation, complete-row checks, nested blocks in cells |
...
| Barcodes | validated EAN-13, Code 128, and QR (versions 1–16) with independent decode tests |
...
| Font fallback stacks | a second embedded face is planned; `.notdef` is the current honest behaviour |
